...
from arbitragelab.codependence.information import variation_of_information_score, get_mutual_info
...
def get_dependence_matrix(df: pd.DataFrame, dependence_method: str, theta: float = 0.5,
    ...
    segment.track('get_dependence_matrix')
    ...
    if dependence_method == 'information_variation':
        ...
        dep_function = lambda x, y: gnpr_distance(x, y, theta=theta)
...
def get_distance_matrix(X: pd.DataFrame, distance_metric: str = 'angular') -> pd.DataFrame:
    ...
    segment.track('get_distance_matrix')
