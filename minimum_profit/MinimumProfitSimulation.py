...
class MinimumProfitSimulation:
    ...
    Methods:
        ...
        simulate_coint(price_params, coint_params, **kwarg): Simulate two cointegrated price series
            with a specified cointegration coefficient.
        verify_ar(price_matrix): Verify if a group of simulated time series are generated by an
            AR(1) process, the output is the mean and standard deviation of the AR(1) coefficient.
        verify_coint(price_series_x, price_series_y, x_name, y_name):
            Verify if a group of simulated price series pairs are cointegrated by a specific
    ...
    def initialize_params() -> Tuple[dict, dict]:
        """
        Initialize the AR(1) process parameter for first-order difference of share S2 price series.
